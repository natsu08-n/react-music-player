...
const useOnClickOutside = ({ref, handler}:UseOnClickOutsideProps): void => {
    useEffect(() => {
        ...
        const listener = (event: MouseEvent | TouchEvent) => {
            //ref.currentが存在しないかFalsyな場合 or event.targetがref.current要素の内部またはそれ自体である場合、returnを返す。
            if(!ref.current || ref.current.contains(event.target as Node)) {
                return;
            }
            //event.targetがref.current要素の外部にある場合に実行
            handler(event);
        };
...
const Volume = ({value, onChange}: VolumeProps) => {
    ...
    useOnClickOutside({ref: divRef, handler: () => setIsOpen(false)});
    ...
    const getVolumeSvg = (volume: number) => {
        if(volume === 0) {
            ...
            return <MediumVolumeSvg />;
        } else {
            return <HighVolumeSvg />;
        }
    };
