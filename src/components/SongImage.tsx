...
// 曲のイメージを表示。また、画像の視覚的なハイライトとなるSVGフレームを備えている。
const SongImage = ({url}: SongImageProps) => {
    ...
    useEffect(() => {
        ...
        if(!imageRef.current) return; //imageRef.currentがnullの場合、関数を終了

        if(url) { //urlがnullではないとき
            imageRef.current.setAttribute("href", url);
            imageRef.current.onload = () => setImageLoaded(true);
        }
    }, [url]);//最初のマウント時とurlに変化があった時にuseEffectを実行
            ...
            >
                ...
                <defs>
                    ...
                    <image
                        ...
                        className="songDetails__image songDetails__skew"
